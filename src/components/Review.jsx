import React from 'react'

const Review = () => {
  return (

  <div className="review" id="Review">
      <h1 className="title">Customer<span>Review</span></h1>

      <div className="review_box">
          <div className="review_card">

              <div className="review_profile">
                  <img src="/image/review_1.webp"/>
              </div>

              <div className="review_text">
                  <h2 className="name">Reviewer 1</h2>

                  <div className="review_icon">
                      <i className="fa-solid fa-star"></i>
                      <i className="fa-solid fa-star"></i>
                      <i className="fa-solid fa-star"></i>
                      <i className="fa-solid fa-star"></i>
                      <i className="fa-solid fa-star-half-stroke"></i>
                  </div>

                  <div className="review_social">
                      <i className="fa-brands fa-facebook-f"></i>
                      <i className="fa-brands fa-instagram"></i>
                      <i className="fa-brands fa-twitter"></i>
                      <i className="fa-brands fa-linkedin-in"></i>
                  </div>

                  <p>
                    Found this place randomly and loved it! Cozy vibe, amazing service, and the food was fantastic. Their signature dish blew me away. Definitely a must-try.
                  </p>

              </div>

          </div>

          <div className="review_card">

              <div className="review_profile">
                  <img src="/image/review_2.webp"/>
              </div>

              <div className="review_text">
                  <h2 className="name">Reviewer 2</h2>

                  <div className="review_icon">
                      <i className="fa-solid fa-star"></i>
                      <i className="fa-solid fa-star"></i>
                      <i className="fa-solid fa-star"></i>
                      <i className="fa-solid fa-star"></i>
                      <i className="fa-solid fa-star-half-stroke"></i>
                  </div>

                  <div className="review_social">
                      <i className="fa-brands fa-facebook-f"></i>
                      <i className="fa-brands fa-instagram"></i>
                      <i className="fa-brands fa-twitter"></i>
                      <i className="fa-brands fa-linkedin-in"></i>
                  </div>

                  <p>
                  The food here is a culinary masterpiece. Each dish was beautifully crafted and tasted even better than it looked. The ambiance was charming and the service was top-notch.
                  </p>

              </div>

          </div>

          <div className="review_card">

              <div className="review_profile">
                  <img src="/image/review_3.webp"/>
              </div>

              <div className="review_text">
                  <h2 className="name">Reviewer 3</h2>

                  <div className="review_icon">
                      <i className="fa-solid fa-star"></i>
                      <i className="fa-solid fa-star"></i>
                      <i className="fa-solid fa-star"></i>
                      <i className="fa-solid fa-star"></i>
                      <i className="fa-solid fa-star-half-stroke"></i>
                  </div>

                  <div className="review_social">
                      <i className="fa-brands fa-facebook-f"></i>
                      <i className="fa-brands fa-instagram"></i>
                      <i className="fa-brands fa-twitter"></i>
                      <i className="fa-brands fa-linkedin-in"></i>
                  </div>

                  <p>
                    Not just food but the service was also impeccable, with staff who went above and beyond to ensure we had a perfect dining experience. Highly recommend for any special occasion.
                    </p>

              </div>

          </div>

          <div className="review_card">

              <div className="review_profile">
                  <img src="/image/review_4.webp"/>
              </div>
              <div className="review_text">
                  <h2 className="name">Reviewer 4</h2>

                  <div className="review_icon">
                      <i className="fa-solid fa-star"></i>
                      <i className="fa-solid fa-star"></i>
                      <i className="fa-solid fa-star"></i>
                      <i className="fa-solid fa-star"></i>
                      <i className="fa-solid fa-star-half-stroke"></i>
                  </div>

                  <div className="review_social">
                      <i className="fa-brands fa-facebook-f"></i>
                      <i className="fa-brands fa-instagram"></i>
                      <i className="fa-brands fa-twitter"></i>
                      <i className="fa-brands fa-linkedin-in"></i>
                  </div>

                  <p>
                    This restaurant sets the standard for fine dining. The food was exquisite, prepared with fresh ingredients and bursting with flavor.
                  </p>

              </div>

          </div>

      </div>

  </div>
  )
}

export default Review